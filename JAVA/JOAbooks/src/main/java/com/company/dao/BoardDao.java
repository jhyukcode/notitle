...
//	2. 최신글읽기 - select * from board order by bno desc
     ...
     try {
   	  ...
   	  while (rset.next()) { result.add (new BoardVO(
   			  rset.getInt("bno"), rset.getString("btitle"), rset.getString("bcontent"),
 			  rset.getString("bname"), rset.getString("bdate"), rset.getInt("bhit"),
 			  rset.getString("bip")));}
